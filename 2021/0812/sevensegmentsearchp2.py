...
def parse_segment(segment):
    ...
    for s in segment:
        if len(s) == 6 and len(superior_replace(digits[4], s)) == 2:
            digits[9] = s
        elif len(s) == 6 and not (digits[1][0] in s and digits[1][1] in s):
            digits[6] = s
        elif len(s) == 6 and not s == digits[6] and not s == digits[9]:
            digits[0] = s
        elif len(s) == 5 and len(superior_replace(digits[1], s)) == 3:
            digits[3] = s
        elif len(s) == 5 and len(superior_replace(digits[7], superior_replace(digits[4], s))) == 1:
            digits[5] = s    
